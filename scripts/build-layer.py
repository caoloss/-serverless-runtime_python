...
PROJECT_SCRIPTS_PATH = pathlib.Path(__file__).parent.relative_to(pathlib.Path(".").absolute())
...
ASSETS_EXTENSION_PATH = ASSETS_PATH.joinpath(EXTENSION_PATH_NAME)
...
def main():
    ...
    logger.debug("copytree:%s:%s", ASSETS_EXTENSION_PATH, BUILD_EXTENSION_PATH)
    shutil.copytree(
        str(ASSETS_EXTENSION_PATH), str(BUILD_EXTENSION_PATH), dirs_exist_ok=True, copy_function=shutil.copy2
    )
